...
const KEY='cash_chain_4315_4340_4315_4215_20260908';
...
async function balance(){const x=await calculateCashBalance({token,service});if(x.requires_baseline||!x.available)throw new Error('CASH_BALANCE_UNAVAILABLE');return{amount:r2(x.cash),source:x.source||null,totals:x.totals||null}}
async function waitBalance(expected){let last=null;for(let i=0;i<15;i++){last=await balance();if(Math.abs(last.amount-expected)<0.005)return last;await sleep(2000)}throw new Error(`BALANCE_MISMATCH expected=${expected} actual=${last?.amount}`)}
...
async function waitSale(msId){for(let i=0;i<10;i++){const q=await service.from('pos_sales').select('*').eq('moysklad_sale_id',msId).maybeSingle();if(q.error)throw q.error;if(q.data)return q.data;await syncMoySkladReceipts({days:2,maxSales:300});await sleep(1000)}throw new Error('SALE_NOT_IMPORTED')}
async function recordReturn(ctx,saleRow,ret,amount){
  ...
  const items=[{position_id:`selftest2:${ctx.item.id}`,catalog_id:ctx.item.id,name:ctx.item.name,qty:1,price:25}];
...
async function run(){
  const prior=await state();if(prior?.status==='PASS'||prior?.status==='RUNNING'||prior?.status==='ABORTED'||prior?.status==='FAIL')return;
  ...
  try{
    ...
    const start=await balance();trace.steps.push({step:'start',expected:4315,actual:start.amount,source:start.source,totals:start.totals});if(Math.abs(start.amount-4315)>=0.005){trace.status='ABORTED';trace.error=`START_BALANCE_MISMATCH expected=4315 actual=${start.amount}`;return}
    ...
    const b1=await waitBalance(4340);trace.steps.push({step:'sale_25',expected:4340,actual:b1.amount,source:b1.source,totals:b1.totals});saleRow=await waitSale(trace.sale.moysklad_id);trace.sale.hub_id=saleRow.id;
    ...
    const b2=await waitBalance(4315);trace.steps.push({step:'return_25',expected:4315,actual:b2.amount,source:b2.source,totals:b2.totals});const op=await cashOut(ctx);trace.cashout={moysklad_id:idOf(op),name:op.name||null,amount:100};
    const b3=await waitBalance(4215);trace.steps.push({step:'cashout_100',expected:4215,actual:b3.amount,source:b3.source,totals:b3.totals});trace.status='PASS';
  }catch(e){trace.status='FAIL';trace.error=String(e?.message||e);if(sale&&!returned){try{const ctx=await context();saleRow=saleRow||await waitSale(idOf(sale));const rr=await makeReturn(ctx,saleRow,'Автовозврат после остановки контрольного теста A4PRINT HUB');trace.compensation={success:true,moysklad_id:idOf(rr.ret),amount:25};trace.compensation.restored_balance=(await waitBalance(4315)).amount}catch(re){trace.compensation={success:false,error:String(re?.message||re)}}}}
  finally{trace.finished_at=new Date().toISOString();await save(trace);console.log('[A4 CASH CHAIN SELFTEST 2]',JSON.stringify(trace))}
}

if(service&&token&&port===internalPort)setTimeout(()=>run().catch(async e=>{await save({status:'FAIL',error:String(e?.message||e),finished_at:new Date().toISOString()});console.error('[A4 CASH CHAIN SELFTEST 2]',e)}),18000);
