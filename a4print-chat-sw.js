const CACHE='a4print-hub-chat-v2';
...
self.addEventListener('fetch',event=>{
  ...
  if(req.mode==='navigate'){
    event.respondWith(fetch(req,{cache:'no-store'}).catch(async()=>await caches.match('/chat/index.html')||Response.error()));return;
  }
  if(url.pathname.startsWith('/chat/')){
    event.respondWith((async()=>{
      try{
        const fresh=await fetch(req,{cache:'no-store'});
        if(fresh&&fresh.ok){const copy=fresh.clone();caches.open(CACHE).then(c=>c.put(req,copy)).catch(()=>{})}
        return fresh;
      }catch{
        return await caches.match(req)||await caches.match(url.pathname)||Response.error();
      }
    })());
  }
});
