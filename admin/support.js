...
function showBotAnswer(html){const el=$('botAnswer');el.innerHTML=html;el.classList.add('show')}
function askBot(){
  const q=$('botQuestion').value.trim();if(!q)return;
  ...
  if(best&&best.score>=4){
    ...
    showBotAnswer(`<b>Точного ответа в базе знаний не нашёл.</b><div>Передам вопрос сотруднику службы поддержки. Нажмите кнопку ниже, чтобы открыть личный чат с оператором.</div><div class="bot-fallback"><button type="button" class="btn green" data-still-help>Позвать оператора</button></div>`);
    document.querySelector('[data-still-help]')?.addEventListener('click',()=>openOperator(q));
  }
}

async function findOwnTicket(){
  const {data,error}=await supabase.from('support_tickets').select('id,subject,status,priority,assigned_to,created_at,updated_at').eq('requester_id',profile.id).in('status',['NEW','OPEN','WAITING']).order('created_at',{ascending:false}).limit(1).maybeSingle();
...
function renderOwnTicketHead(){
  if(!currentTicket){$('ticketTitle').textContent='Оператор не вызван';$('ticketStatus').textContent='Сначала задайте вопрос помощнику или нажмите «Позвать оператора»';$('ticketText').disabled=true;$('ticketSend').disabled=true;return}
...
async function loadOwnMessages(){
  ...
  if(error)throw error;renderMessages($('ticketMessages'),data||[],true);
}
function renderMessages(host,rows,ownMode=false){
...
async function loadOperatorTickets(keep=true){
  const {data,error}=await supabase.from('support_tickets').select('id,requester_id,subject,status,priority,assigned_to,source,created_at,updated_at,users:requester_id(full_name,email)').order('updated_at',{ascending:false}).limit(300);
...
async function openOperatorTicket(id){
  ...
  $('operatorTicketMeta').textContent=`${operatorTicket.subject} · ${fmt(operatorTicket.created_at)}`;$('operatorStatus').value=operatorTicket.status;
...
async function saveOperatorStatus(){
  if(!operatorTicket)return;const status=$('operatorStatus').value;const patch={status,updated_at:new Date().toISOString(),assigned_to:profile.id,closed_at:['RESOLVED','CLOSED'].includes(status)?new Date().toISOString():null};
...
async function init(){
  ...
  const isOperator=roles.includes('SUPPORT')||roles.includes('ADMIN');
  $('supportRole').textContent=isOperator?(roles.includes('ADMIN')?'Администратор · поддержка':'Оператор поддержки'):(profile.full_name||profile.email||'Сотрудник');
  await loadFaq();
  if(isOperator){
...
$('faqSearch').oninput=renderFaq;$('askBot').onclick=askBot;$('botQuestion').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();askBot()}});
$('callOperatorTop').onclick=()=>openOperator($('botQuestion').value.trim());$('ticketComposer').onsubmit=sendOwn;
$('ticketSearch').oninput=renderOperatorTickets;$('ticketFilter').onchange=renderOperatorTickets;$('reloadTickets').onclick=()=>loadOperatorTickets(true).catch(e=>alert(e.message));$('operatorComposer').onsubmit=sendOperator;$('saveTicketStatus').onclick=saveOperatorStatus;
