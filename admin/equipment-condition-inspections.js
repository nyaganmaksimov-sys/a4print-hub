...
const state={rows:[],targets:[],permissions:[],loading:false,amendmentFilter:new URLSearchParams(location.search).get('amendment')||''};
...
function render(){
  ...
  const rows=state.rows.filter(r=>(!state.amendmentFilter||r.composition_amendment_id===state.amendmentFilter)&&(!type||r.inspection_type===type)&&(!status||r.status===status)&&(!q||`${r.partner_name} ${r.contract_number} ${r.inventory_number} ${r.equipment_name} ${r.composition_amendment_number||''}`.toLowerCase().includes(q)));
...
function rowHtml(r){
  ...
  return `<article class="eci-row"><div><h3>${esc(typeLabel[r.inspection_type]||r.inspection_type)} · ${esc(r.inventory_number||'—')} · ${esc(r.equipment_name||'')}</h3><p>${esc(r.partner_name||'—')} · договор ${esc(r.contract_number||'—')}${r.composition_amendment_number?` · ДС ${esc(r.composition_amendment_number)} · ${r.composition_change_action==='ADD'?'добавление':'вывод'}`:''}</p><small>${r.document_number?`Документ ${esc(r.document_number)} · `:''}${files.length} файл(ов)${defects.length?` · дефектов: ${defects.length}`:''}</small></div><div><small>Дата / место</small><b>${date(r.inspected_on)}</b><p>${esc(r.location||'—')}</p></div><div><small>Состояние</small><b>${esc(gradeLabel[r.condition_grade]||r.condition_grade)}</b><p><span class="eci-pill ${esc(r.operational_state)}">${esc(opLabel[r.operational_state]||r.operational_state)}</span>${r.meter_hours!=null?` · ${Number(r.meter_hours).toLocaleString('ru-RU')} ч`:''}</p></div><div><span class="eci-pill ${esc(r.status)}">${esc(r.status)}</span><div class="eci-actions" style="margin-top:8px">${actions}</div></div></article>`;
}

function fillTargets(selected){
  let targets=[...state.targets];
  const existing=state.rows.find(r=>r.inspection_id===$('eciId').value);
  if(existing&&!targets.some(t=>targetKey(t)===`${existing.contract_id}:${existing.equipment_id}`))targets.push({contract_id:existing.contract_id,equipment_id:existing.equipment_id,partner_name:existing.partner_name,contract_number:existing.contract_number,inventory_number:existing.inventory_number,equipment_name:existing.equipment_name,termination_id:existing.termination_id,composition_amendment_asset_id:existing.composition_amendment_asset_id,composition_amendment_id:existing.composition_amendment_id,composition_change_action:existing.composition_change_action,composition_amendment_number:existing.composition_amendment_number});
...
function openForm(id){
  ...
  $('eciId').value=r?.inspection_id||'';$('eciDlgTitle').textContent=r?.composition_amendment_number?`Осмотр по ДС ${r.composition_amendment_number} · ${r.composition_change_action==='ADD'?'добавление':'вывод'}`:(r?'Изменить черновик':'Новый осмотр');fillTargets(r?`${r.contract_id}:${r.equipment_id}`:'');
  ...
  $('eciTarget').disabled=Boolean(r?.composition_amendment_asset_id);$('eciInspectionType').disabled=Boolean(r?.composition_amendment_asset_id);$('eciInspectedOn').readOnly=Boolean(r?.composition_amendment_asset_id);renderExistingFiles(r?.files||[]);$('eciDlg').showModal();
}
...
function selectedTarget(){const existing=state.rows.find(r=>r.inspection_id===$('eciId').value);if(existing)return{contract_id:existing.contract_id,equipment_id:existing.equipment_id,termination_id:existing.termination_id,composition_amendment_asset_id:existing.composition_amendment_asset_id,composition_amendment_id:existing.composition_amendment_id,composition_change_action:existing.composition_change_action};const key=$('eciTarget').value;const[contract,equipment]=key.split(':');return state.targets.find(t=>t.contract_id===contract&&t.equipment_id===equipment)||state.rows.filter(r=>r.contract_id===contract&&r.equipment_id===equipment).map(r=>({contract_id:r.contract_id,equipment_id:r.equipment_id,termination_id:r.termination_id,composition_amendment_asset_id:r.composition_amendment_asset_id,composition_amendment_id:r.composition_amendment_id,composition_change_action:r.composition_change_action}))[0]}

async function saveDraft(e){
  e.preventDefault();$('eciFormError').innerHTML='';const t=selectedTarget();if(!t){formError('Выберите договор и оборудование.');return}const type=$('eciInspectionType').value;if(type==='RETURN'&&!t.termination_id&&!t.composition_amendment_asset_id){formError('Для обычного возвратного осмотра сначала должен быть открыт процесс расторжения договора. Для вывода по ДС откройте осмотр через кнопку «Осмотры состава».');return}
  const payload={p_inspection_id:$('eciId').value||null,p_contract_id:t.contract_id,p_equipment_id:t.equipment_id,p_inspection_type:type,p_termination_id:type==='RETURN'?(t.termination_id||null):null,p_inspected_on:$('eciInspectedOn').value,p_location:$('eciLocation').value.trim()||null,p_meter_hours:$('eciMeter').value===''?null:Number($('eciMeter').value),p_condition_grade:$('eciGrade').value,p_operational_state:$('eciOperational').value,p_checklist:checklistPayload(),p_defects:defectsPayload(),p_notes:$('eciNotes').value.trim()||null};
...
function friendly(error){const m=String(error?.message||error||'Ошибка');if(m.includes('INSPECTION_PHOTO_REQUIRED'))return 'Для приёмки или возврата нужна минимум одна фотография.';if(m.includes('DEFECT_DESCRIPTION_REQUIRED'))return 'Есть замечание по чек-листу или состоянию — добавьте описание дефекта.';if(m.includes('RETURN_INSPECTION_REQUIRED'))return 'Нельзя завершить возврат договора: не по всем единицам оборудования есть завершённый возвратный осмотр.';if(m.includes('CHECKLIST_REQUIRED'))return 'Заполните чек-лист.';if(m.includes('INSPECTION_NOT_EDITABLE'))return 'Этот осмотр уже нельзя изменять.';if(m.includes('COMPOSITION_INSPECTION_AMENDMENT_MISMATCH'))return 'Фактическое состояние не совпадает с данными подписанного ДС. Отмените ДС и оформите актуальное.';if(m.includes('COMPOSITION_INSPECTION_DATE_MUST_MATCH_AMENDMENT'))return 'Дата осмотра должна совпадать с датой вступления composition-ДС.';if(m.includes('COMPOSITION_INSPECTION_TYPE_IMMUTABLE'))return 'Тип осмотра по composition-ДС менять нельзя.';return m}
