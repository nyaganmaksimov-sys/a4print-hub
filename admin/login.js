...
const config=window.A4PRINT_CONFIG||{},SUPABASE_URL=config.supabaseUrl||'https://qgakliolffnwkymoqvzn.supabase.co',SUPABASE_PUBLISHABLE_KEY=config.supabasePublishableKey||'',RETURN_KEY='a4print_auth_return_to';
...
function validReturn(raw){if(!raw)return null;try{const u=new URL(raw,location.origin);if(u.origin===location.origin&&(u.pathname.startsWith('/chat/')||u.pathname.startsWith('/mobile/')||u.pathname.startsWith('/admin/')))return u.pathname+u.search+u.hash}catch{}return null}
...
async function routeChatReturn(rt){
  ...
  if(u?.is_active===true){clearReturn();location.replace(rt);return true}
  if(u?.is_active===false){clearReturn();location.replace('./pending.html');return true}
  location.replace('./register.html?returnTo='+encodeURIComponent(rt));return true
}
async function routeByProfile(){const rt=safeReturnTo();if(rt)return routeChatReturn(rt);const{data,error:e}=await supabase.rpc('get_my_staff_profile');if(e)throw e;const st=data?.status;if(st==='ACTIVE'){clearReturn();const roles=new Set((data.roles||[]).map(r=>r.name));location.replace(roles.has('POS_OPERATOR')&&roles.size===1?'../pos/index.html':'./index.html');return}if(st==='PENDING'||st==='REJECTED'){clearReturn();location.replace('./pending.html');return}location.replace('./register.html')}
