(()=>{
  ...
  function cancelPlayback(){
    generation+=1;
    const cancel=cancelCurrent;cancelCurrent=null;
    if(cancel)try{cancel()}catch{}
    try{window.speechSynthesis?.cancel?.()}catch{}
    cleanupAudio();setSpeaking(false);
  }
  ...
  function browserVoice(text,profile,myGeneration){
    ...
    return new Promise(resolve=>{
      ...
      const finish=value=>{if(settled)return;settled=true;clearTimeout(timer);if(cancelCurrent===cancel)cancelCurrent=null;resolve(value)};
      const cancel=()=>{try{window.speechSynthesis.cancel()}catch{}finish(false)};
      cancelCurrent=cancel;
      try{
        const utterance=new SpeechSynthesisUtterance(String(text));
        utterance.lang='ru-RU';utterance.rate=profile==='male'?0.92:0.98;utterance.pitch=profile==='male'?0.82:1.02;utterance.volume=1;
        const voices=window.speechSynthesis.getVoices?.()||[];
        const russian=voices.filter(v=>String(v.lang||'').toLowerCase().startsWith('ru'));
        const preferred=russian.find(v=>profile==='male'?/dmit|pavel|alex|male|муж/i.test(v.name):/svetlana|alena|irina|mariya|female|жен/i.test(v.name));
  ...
  async function neuralAudio(item){
    ...
    const response=await fetch(`${base}/api/v1/jarvis/tts`,{
      method:'POST',cache:'no-store',headers:{Authorization:`Bearer ${token}`,'Content-Type':'application/json',Accept:'audio/mpeg'},
      body:JSON.stringify({text:String(item.text).slice(0,2500),voice:item.profile})
    });
    if(!response.ok){const body=await response.json().catch(()=>({}));throw new Error(body.error||body.detail||`VOICE_HTTP_${response.status}`)}
  ...
  async function playItem(item){
    ...
    try{
      const blob=await neuralAudio(item);if(myGeneration!==generation)return false;
      cleanupAudio();currentUrl=URL.createObjectURL(blob);const audio=new Audio(currentUrl);currentAudio=audio;
      const played=await new Promise((resolve,reject)=>{
        let settled=false;
        const finish=(value,error)=>{if(settled)return;settled=true;if(cancelCurrent===cancel)cancelCurrent=null;error?reject(error):resolve(value)};
        const cancel=()=>finish(false);cancelCurrent=cancel;
        audio.onended=()=>finish(true);audio.onerror=()=>finish(false,new Error('VOICE_AUDIO_PLAYBACK_FAILED'));
        const started=audio.play();if(started?.catch)started.catch(error=>finish(false,error));
      });
      cleanupAudio();return played;
    }catch(error){
      cleanupAudio();if(myGeneration!==generation)return false;if(item.fallback===false)throw error;
  ...
  async function drain(){
    ...
    try{
      while(queue.length){
        ...
        setSpeaking(true);
        try{item.resolve(await playItem(item))}catch(error){item.reject(error)}finally{setSpeaking(false)}
      }
    }finally{running=false;cancelCurrent=null;setSpeaking(false)}
  }
