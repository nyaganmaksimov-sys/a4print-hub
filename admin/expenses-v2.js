...
function applyAccessUi(){
  if(state.leader){
    ...
    $('accessHint').textContent='Руководитель видит все суммы, чеки и может редактировать записи.';
    ...
    $('accessHint').textContent='Свои суммы и чеки видны полностью. У чужих расходов сумма и чек скрыты.';
...
function filtered(){
  ...
  return state.items.filter(x=>{const text=[x.title,x.category,x.payment_method,x.counterparty,x.note,x.created_by_name].filter(Boolean).join(' ').toLowerCase();return(!q||text.includes(q))&&(!category||x.category===category)&&(!status||String(x.status||'').toUpperCase()===status)})
}
...
async function saveExpense(event){
  ...
  try{
    ...
    button.textContent=state.editingId?'Сохранение…':'Сохранение…';
