...
function statusSelect(p){return canManage()?`<select data-payment-status="${p.id}" style="min-height:32px;border:1px solid #cbd5e1;border-radius:9px;background:#fff;padding:0 7px;font-size:10px;font-weight:800"><option value="PENDING" ${p.status==='PENDING'?'selected':''}>Ожидается</option><option value="CONFIRMED" ${p.status==='CONFIRMED'?'selected':''}>Подтверждён</option><option value="CANCELLED" ${p.status==='CANCELLED'?'selected':''}>Отменён</option><option value="REFUNDED" ${p.status==='REFUNDED'?'selected':''}>Возвращён</option></select>`:`<span class="cf-badge ${statusTone(p.status)}">${esc(p.status||'PENDING')}</span>`}
function row(p){
...
function openPayment(customerId=''){
  if(!canManage())return;$('paymentForm').reset();$('paymentError').textContent='';$('paymentStatus').value='CONFIRMED';$('paymentMethod').value='Банк';$('paymentDate').value=new Date().toISOString().slice(0,16);fillCustomerOptions();if(customerId&&[...$('paymentCustomer').options].some(o=>o.value===customerId))$('paymentCustomer').value=customerId;refreshLinkedOptions();$('paymentDlg').showModal();
}
function autoOpenFromUrl(){if(state.autoOpened||!canManage())return;const customer=new URLSearchParams(location.search).get('customer');if(customer){state.autoOpened=true;openPayment(customer)}}
...
$('paymentForm').addEventListener('submit',async e=>{
  ...
  const payload={organization_id:state.organizationId,customer_id:$('paymentCustomer').value||null,order_id:$('paymentOrder').value||null,customer_document_id:$('paymentDocument').value||null,payment_type:'INCOMING',status,amount,currency:'RUB',payment_method:$('paymentMethod').value||null,paid_at:isSuccess({status})?new Date($('paymentDate').value||Date.now()).toISOString():null,note:$('paymentNote').value.trim()||null,created_by:state.profileId,updated_at:new Date().toISOString()};
