...
if(!window.__A4_EQUIPMENT_ECONOMICS__){
  ...
  function aggregateCapacity(rows){
    const map=new Map();
    for(const r of rows||[])map.set(r.equipment_id,Number(map.get(r.equipment_id)||0)+Number(r.available_minutes||0));
    return map;
  }
  function ensure(){
    ...
    root.innerHTML=`<div class="eq-head econ-head"><div><h2>📊 Экономика производственной фермы</h2><p>Выручка, себестоимость, маржа, машинное время, сервис, окупаемость и обязательства владельцам.</p></div><div class="econ-controls"><select id="econPeriod"><option value="30">30 дней</option><option value="90">90 дней</option><option value="365">365 дней</option><option value="0">Всё время</option></select><button id="econRefresh" type="button">↻ Обновить</button></div></div><div class="econ-note">Загрузка считается по <b>календарю мощности оборудования</b>, который настраивается в разделе «Производство → План загрузки». Для существующих станков стартовое значение сохранено как 8 часов Пн–Пт.</div><div id="econSummary" class="econ-summary"><div class="eq-empty">Загрузка…</div></div><div id="econTable" class="econ-table-wrap"><div class="eq-empty">Загрузка аналитики…</div></div>`;
  ...
  async function load(){
    ...
    try{
      const days=Number(document.getElementById('econPeriod')?.value||30),from=startFor(days),yearFrom=startFor(365),today=new Date().toISOString().slice(0,10);
      const capacityRequest=days?supabase.rpc('production_capacity_between',{p_from:from,p_to:today}):Promise.resolve({data:[],error:null});
      const [periodRows,yearRows,allRows,assetsR,obligationsR,capacityR]=await Promise.all([
      ...
      if(assetsR.error)throw assetsR.error;if(obligationsR.error)throw obligationsR.error;if(capacityR.error)throw capacityR.error;
      const period=aggregate(periodRows),year=aggregate(yearRows),all=aggregate(allRows),ob=new Map((obligationsR.data||[]).map(x=>[x.equipment_id,x])),capacity=aggregateCapacity(capacityR.data||[]);
      ...
      const rows=assets.map(a=>{
        ...
        const revenue=Number(p.operation_revenue||0),prod=Number(p.production_cost||0),net=Number(p.net_margin_after_service||0),minutes=Number(p.machine_minutes||0),available=Number(capacity.get(a.id)||0);
        const util=days&&available>0?Math.min(999,minutes/available*100):days&&minutes>0?999:null;
  ...
  function realtime(){
    ...
    let ch=supabase.channel('equipment-economics-v2');
    for(const table of ['production_jobs','equipment_service_log','equipment_assets','equipment_owner_settlements','equipment_lease_charges','equipment_capacity_rules'])ch=ch.on('postgres_changes',{event:'*',schema:'public',table},reload);
