async function initA4Navigation(){
  ...
  const nav=sidebar.querySelector('nav');if(!nav)return;const path=location.pathname.split('/').pop()||'index.html';const items=[['index.html','home','Главная'],['manager.html','manager','Менеджер'],['orders.html','orders','Заказы'],['customers.html','customers','Клиенты'],['partners.html','partners','Партнёры'],['requests.html','applications','Заявки'],['users.html','users','Пользователи'],['warehouse.html','warehouse','Склад и номенклатура'],['production.html','production','Производство'],['messages.html','messages','Сообщения'],['documents.html','documents','Документы'],['payments.html','payments','Оплаты'],['reports.html','reports','Отчёты'],['settings.html','settings','Настройки'],['help.html','help','Инструкция'],['../index.html','systems','Выбор системы']];nav.innerHTML=items.map(([href,icon,label])=>`<a href="${href.startsWith('..')?href:'./'+href}" class="${path===href?'active':''}" title="${href==='manager.html'?'Рабочий стол менеджера':label}"><span class="a4-nav-icon">${icons[icon]}</span><span class="a4-nav-label">${label}</span>${href==='orders.html'?'<b id="orderCount">0</b>':''}${href==='requests.html'?'<b id="requestCount" style="display:none">0</b>':''}</a>`).join('');
  try{const cfg=window.A4PRINT_CONFIG||{};if(cfg.supabaseUrl&&cfg.supabasePublishableKey){const{createClient}=await import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm');const supabase=createClient(cfg.supabaseUrl,cfg.supabasePublishableKey);const [{count:c1},{count:c2}]=await Promise.all([supabase.from('connection_requests').select('*',{count:'exact',head:true}).eq('status','NEW'),supabase.from('executor_registration_requests').select('*',{count:'exact',head:true}).eq('status','NEW')]);const total=(c1||0)+(c2||0),badge=document.getElementById('requestCount');if(badge&&total){badge.textContent=total;badge.style.display='inline-flex'}}}catch(e){console.warn('Не удалось загрузить счётчик заявок',e)}
}
