...
const addDays=(date,days)=>{const d=new Date(`${date}T00:00:00`);if(!Number.isFinite(d.getTime()))return'';d.setDate(d.getDate()+Number(days||0));return d.toISOString().slice(0,10)};
...
async function context(){
  ...
  state.profileId=profile.data?.id||null;
}
...
async function load(){
  ...
  try{
    const [assets,consumables,balances,links,movements,services,users,organizations]=await Promise.all([
      ...
      supabase.from('equipment_service_log').select('*').order('serviced_at',{ascending:false}).order('created_at',{ascending:false}).limit(2000),
    ...
    state.consumables=consumables.data||[];
    ...
    fillSelects();
    renderAll();
    $('addEquipment').hidden=!canManage();
    $('addConsumable').hidden=!canManage();
    $('addMovement').hidden=!canMove();
    $('addMovement2').hidden=!canMove();
    ...
    $('equipmentGrid').innerHTML=`<div class="eq-empty" style="color:#b91c1c">Ошибка загрузки: ${esc(error.message||error)}</div>`;
  }finally{
    state.loading=false;
...
function fillSelects(){
  ...
  $('eqOrganization').innerHTML=orgOptions;
  $('conOrganization').innerHTML=orgOptions;
  ...
  $('equipmentOrg').value=currentEqOrg;
  $('consumableOrg').value=currentConOrg;
  ...
  $('moveEquipmentSelect').innerHTML=assetOptions;
  $('movementEquipment').innerHTML='<option value="">Любое оборудование</option>'+state.assets.map(a=>`<option value="${a.id}">${esc(a.inventory_number)} · ${esc(a.name)}</option>`).join('');
...
function renderKpis(){
  ...
  $('equipmentCount').textContent=state.assets.length;
  $('activeCount').textContent=state.assets.filter(a=>a.status==='ACTIVE').length;
  $('serviceCount').textContent=service;
  $('lowStockCount').textContent=state.balances.filter(b=>b.is_active&&b.low_stock).length;
}
...
function renderEquipment(){
  ...
  $('equipmentGrid').innerHTML=rows.map(a=>{const u=userBy(a.responsible_user_id),s=serviceState(a);return `<article class="eq-asset" tabindex="0" data-asset="${a.id}"><div class="eq-asset-top"><div><div class="eq-inv">${esc(a.inventory_number)}</div><h3>${esc(a.name)}</h3><div class="eq-muted">${esc([a.brand,a.model].filter(Boolean).join(' ')||a.category||'Категория не указана')}</div></div><span class="eq-badge ${statusTone(a.status)}">${esc(statusLabels[a.status]||a.status)}</span></div><div class="eq-meta"><div><span>Место</span><b>${esc(a.location||'—')}</b></div><div><span>Ответственный</span><b>${esc(u?.full_name||u?.email||'—')}</b></div><div><span>Последнее ТО</span><b>${esc(fmtDate(a.last_service_date))}</b></div><div><span>Сервис</span><b class="${s.tone==='red'?'eq-service-due':''}">${esc(s.label)}</b></div></div></article>`}).join('')||'<div class="eq-empty">Оборудование не найдено</div>';
}
...
function openEquipment(a=null){
  ...
  $('equipmentId').value=a?.id||'';
  $('equipmentDlgTitle').textContent=a?'Редактировать оборудование':'Новое оборудование';
  $('equipmentDlgText').textContent=a?`Инвентарный номер ${a.inventory_number}`:'Инвентарный номер будет присвоен автоматически.';
  const defaultOrg=state.organizations.find(o=>o.code==='A4PRINT')?.id||'';
  $('eqOrganization').value=a?.organization_id||defaultOrg;
  $('eqName').value=a?.name||'';
  $('eqCategory').value=a?.category||'';
  $('eqBrand').value=a?.brand||'';
  $('eqModel').value=a?.model||'';
  $('eqSerial').value=a?.serial_number||'';
  $('eqLocation').value=a?.location||'';
  $('eqResponsible').value=a?.responsible_user_id||'';
  $('eqStatus').value=a?.status||'ACTIVE';
  $('eqPurchaseDate').value=a?.purchase_date||'';
  $('eqPurchasePrice').value=a?.purchase_price??'';
  $('eqWarranty').value=a?.warranty_until||'';
  $('eqServiceInterval').value=a?.service_interval_days??'';
  $('eqLastService').value=a?.last_service_date||'';
  $('eqNextService').value=a?.next_service_date||'';
  $('eqNotes').value=a?.notes||'';
  $('equipmentDlg').showModal();
}

async function saveEquipment(e){
  e.preventDefault();
  if(!canManage())return;
  const b=$('saveEquipment');
  b.disabled=true;
  $('equipmentError').textContent='';
  const id=$('equipmentId').value;
  let next=$('eqNextService').value||null;
  const last=$('eqLastService').value,interval=Number($('eqServiceInterval').value||0);
  if(!next&&last&&interval>0)next=addDays(last,interval);
  ...
  try{
    const q=id?supabase.from('equipment_assets').update(payload).eq('id',id):supabase.from('equipment_assets').insert(payload);
    const {error}=await q;
    if(error)throw error;
    $('equipmentDlg').close();
    await load();
  }catch(err){$('equipmentError').textContent=err.message||String(err)}finally{b.disabled=false}
}

function openConsumable(c=null){
  if(!canManage())return;
  $('consumableForm').reset();
  $('consumableError').textContent='';
  $('consumableId').value=c?.id||'';
  $('consumableDlgTitle').textContent=c?'Редактировать расходник':'Новый расходник';
  const defaultOrg=state.organizations.find(o=>o.code==='A4PRINT')?.id||'';
  $('conOrganization').value=c?.organization_id||defaultOrg;
  $('conName').value=c?.name||'';
  $('conCategory').value=c?.category||'';
  $('conSku').value=c?.sku||'';
  $('conUnit').value=c?.unit||'шт';
  $('conMinStock').value=c?.min_stock??0;
  $('conStorage').value=c?.storage_location||'';
  $('conSupplier').value=c?.supplier||'';
  $('conActive').value=c?.is_active===false?'0':'1';
  $('conNotes').value=c?.notes||'';
  $('consumableDlg').showModal();
}

async function saveConsumable(e){
  e.preventDefault();
  if(!canManage())return;
  const b=$('saveConsumable');b.disabled=true;$('consumableError').textContent='';
  const id=$('consumableId').value,payload={organization_id:$('conOrganization').value||null,name:$('conName').value.trim(),category:$('conCategory').value.trim()||null,sku:$('conSku').value.trim()||null,unit:$('conUnit').value.trim()||'шт',min_stock:Number($('conMinStock').value||0),storage_location:$('conStorage').value.trim()||null,supplier:$('conSupplier').value.trim()||null,is_active:$('conActive').value==='1',notes:$('conNotes').value.trim()||null};
  if(!id)payload.created_by=state.profileId;
  try{
    const q=id?supabase.from('equipment_consumables').update(payload).eq('id',id):supabase.from('equipment_consumables').insert(payload);
    const {error}=await q;
    if(error)throw error;
    $('consumableDlg').close();
    await load();
  }catch(err){$('consumableError').textContent=err.message||String(err)}finally{b.disabled=false}
}

function openMovement(consumableId='',equipmentId=''){
  if(!canMove())return;
  $('movementForm').reset();$('movementError').textContent='';$('moveType').value='RECEIPT';$('moveQty').value='1';
  fillSelects();
  if(consumableId)$('moveConsumable').value=consumableId;
  if(equipmentId)$('moveEquipmentSelect').value=equipmentId;
  updateMovementHint();
  $('movementDlg').showModal();
}
...
async function saveMovement(e){
  e.preventDefault();
  if(!canMove())return;
  const b=$('saveMovement');b.disabled=true;$('movementError').textContent='';
  const type=$('moveType').value,raw=Number($('moveQty').value);
  if(!raw||(type!=='ADJUSTMENT'&&raw<0)){$('movementError').textContent='Проверьте количество.';b.disabled=false;return}
  let delta=raw;
  if(type==='ISSUE'||type==='WRITE_OFF')delta=-Math.abs(raw);else if(type==='RECEIPT'||type==='RETURN')delta=Math.abs(raw);
  const payload={consumable_id:$('moveConsumable').value,equipment_id:$('moveEquipmentSelect').value||null,movement_type:type,quantity_delta:delta,note:$('moveNote').value.trim()||null,document_ref:$('moveDocument').value.trim()||null,created_by:state.profileId};
  try{
    const {error}=await supabase.from('equipment_consumable_movements').insert(payload);
    if(error)throw error;
    $('movementDlg').close();
    await load();
  }catch(err){$('movementError').textContent=String(err.message||err).replace('INSUFFICIENT_CONSUMABLE_STOCK:','Недостаточный остаток:')}finally{b.disabled=false}
}

function openService(assetId){
  if(!canManage())return;
  const a=assetBy(assetId);if(!a)return;
  $('serviceForm').reset();
  $('serviceError').textContent='';
  $('serviceEquipmentId').value=assetId;
  $('serviceDlgText').textContent=`${a.inventory_number} · ${a.name}`;
  $('serviceType').value='MAINTENANCE';
  $('serviceDate').value=isoToday();
  $('serviceCost').value='';
  $('serviceProvider').value='';
  $('serviceDescription').value='';
  $('serviceNext').value=a.service_interval_days?addDays(isoToday(),a.service_interval_days):(a.next_service_date||'');
  $('serviceDlg').showModal();
}

async function saveService(e){
  e.preventDefault();
  if(!canManage())return;
  const b=$('saveService');b.disabled=true;$('serviceError').textContent='';
  const equipmentId=$('serviceEquipmentId').value;
  const servicedAt=$('serviceDate').value;
  const description=$('serviceDescription').value.trim();
  if(!equipmentId||!servicedAt||!description){$('serviceError').textContent='Укажите дату и что было сделано.';b.disabled=false;return}
  const costRaw=$('serviceCost').value.trim();
  const payload={equipment_id:equipmentId,service_type:$('serviceType').value,serviced_at:servicedAt,description,cost:costRaw===''?null:Number(costRaw),next_due_date:$('serviceNext').value||null,provider:$('serviceProvider').value.trim()||null,created_by:state.profileId};
  try{
    const {error}=await supabase.from('equipment_service_log').insert(payload);
    if(error)throw error;
    $('serviceDlg').close();
    await load();
  }catch(err){$('serviceError').textContent=err.message||String(err)}finally{b.disabled=false}
}

function openDetail(id){state.selectedAssetId=id;renderDetail(id);$('detailDlg').showModal()}
function renderDetail(id){
  const a=assetBy(id);if(!a)return;
  const u=userBy(a.responsible_user_id),o=orgBy(a.organization_id),linked=state.links.filter(l=>l.equipment_id===id),moves=state.movements.filter(m=>m.equipment_id===id).slice(0,20),services=state.services.filter(s=>s.equipment_id===id),available=state.consumables.filter(c=>c.is_active&&!linked.some(l=>l.consumable_id===c.id));
  const serviceCost=services.reduce((sum,s)=>sum+Number(s.cost||0),0);
  $('detailBody').innerHTML=`<div class="eq-detail-hero"><div><div class="eq-inv">${esc(a.inventory_number)}</div><h3>${esc(a.name)}</h3><div class="eq-muted">${esc([a.brand,a.model,a.serial_number?`S/N ${a.serial_number}`:''].filter(Boolean).join(' · '))}</div></div><div class="eq-actions">${canManage()?`<button type="button" data-edit-asset="${a.id}">Изменить</button><button class="primary" type="button" data-open-service="${a.id}">+ Записать ТО</button>`:''}${canMove()?`<button class="green" type="button" data-issue-asset="${a.id}">Выдать расходник</button>`:''}</div></div>
  <div class="eq-meta"><div><span>Статус</span><b>${esc(statusLabels[a.status]||a.status)}</b></div><div><span>Подразделение</span><b>${esc(o?.name||'—')}</b></div><div><span>Место</span><b>${esc(a.location||'—')}</b></div><div><span>Ответственный</span><b>${esc(u?.full_name||u?.email||'—')}</b></div><div><span>Покупка</span><b>${esc(fmtDate(a.purchase_date))}${a.purchase_price!=null?' · '+esc(money(a.purchase_price)):''}</b></div><div><span>Гарантия</span><b>${esc(fmtDate(a.warranty_until))}</b></div><div><span>Последнее обслуживание</span><b>${esc(fmtDate(a.last_service_date))}</b></div><div><span>Следующее ТО</span><b>${esc(fmtDate(a.next_service_date))}</b></div></div>
  ${a.notes?`<div class="eq-detail-section"><h4>Комментарий</h4><div class="eq-muted">${esc(a.notes)}</div></div>`:''}
  <div class="eq-detail-section"><h4>Совместимые расходники</h4><div class="eq-mini-list">${linked.length?linked.map(l=>{const c=consumableBy(l.consumable_id),b=balanceBy(l.consumable_id);return `<div class="eq-mini"><span><b>${esc(c?.name||'Расходник')}</b><small>${esc(l.note||c?.sku||'')}</small></span><strong class="${b?.low_stock?'eq-service-due':''}">${qty(b?.current_stock||0)} ${esc(c?.unit||'')}</strong></div>`}).join(''):'<div class="eq-empty" style="padding:16px">Расходники ещё не привязаны</div>'}</div>${canManage()?`<div class="eq-inline" style="margin-top:9px"><label class="eq-field"><span>Добавить совместимый расходник</span><select id="detailConsumable"><option value="">Выберите…</option>${available.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}</select></label><button class="primary" type="button" data-link-consumable="${a.id}">Привязать</button></div>`:''}</div>
  <div class="eq-detail-section"><div style="display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap"><h4 style="margin:0">ТО и обслуживание</h4><div class="eq-muted">Записей: <b>${services.length}</b> · затраты: <b>${esc(money(serviceCost))}</b></div></div><div class="eq-mini-list" style="margin-top:9px">${services.length?services.map(s=>{const creator=userBy(s.created_by);return `<div class="eq-mini"><span><b>${esc(serviceLabels[s.service_type]||s.service_type)} · ${esc(fmtDate(s.serviced_at))}</b><small>${esc(s.description)}${s.provider?' · исполнитель: '+esc(s.provider):''}${s.next_due_date?' · следующее ТО '+esc(fmtDate(s.next_due_date)):''}${creator?' · записал '+esc(creator.full_name||creator.email):''}</small></span><strong>${s.cost!=null?esc(money(s.cost)):'—'}</strong></div>`}).join(''):'<div class="eq-empty" style="padding:16px">Записей обслуживания нет</div>'}</div>${canManage()?`<div class="eq-inline" style="margin-top:9px"><button class="primary" type="button" data-open-service="${a.id}">+ Записать ТО / ремонт</button></div>`:''}</div>
  <div class="eq-detail-section"><h4>Последние движения расходников</h4><div class="eq-mini-list">${moves.length?moves.map(m=>{const c=consumableBy(m.consumable_id),pos=Number(m.quantity_delta)>0;return `<div class="eq-mini"><span><b>${esc(c?.name||'Расходник')} · ${esc(moveLabels[m.movement_type]||m.movement_type)}</b><small>${esc(fmtDateTime(m.created_at))}${m.note?' · '+esc(m.note):''}</small></span><strong class="${pos?'eq-move-plus':'eq-move-minus'}">${pos?'+':''}${qty(m.quantity_delta)} ${esc(c?.unit||'')}</strong></div>`}).join(''):'<div class="eq-empty" style="padding:16px">Движений нет</div>'}</div></div>`;
}

async function linkConsumable(assetId){
  const cid=$('detailConsumable')?.value;if(!cid)return;
  const {error}=await supabase.from('equipment_consumable_links').insert({equipment_id:assetId,consumable_id:cid,created_by:state.profileId});
...
function switchTab(name){
  ...
  for(const [n,id] of [['equipment','equipmentTab'],['consumables','consumablesTab'],['movements','movementsTab']])$(id).classList.toggle('eq-hidden',n!==name);
}

$('refresh').addEventListener('click',load);
$('addEquipment').addEventListener('click',()=>openEquipment());
$('addConsumable').addEventListener('click',()=>openConsumable());
$('addMovement').addEventListener('click',()=>openMovement());
$('addMovement2').addEventListener('click',()=>openMovement());
$('equipmentForm').addEventListener('submit',saveEquipment);
$('consumableForm').addEventListener('submit',saveConsumable);
$('movementForm').addEventListener('submit',saveMovement);
$('serviceForm').addEventListener('submit',saveService);
$('moveConsumable').addEventListener('change',updateMovementHint);
$('moveType').addEventListener('change',updateMovementHint);
...
for(const id of ['consumableSearch','stockFilter','consumableOrg'])$(id).addEventListener(id==='consumableSearch'?'input':'change',renderConsumables);
for(const id of ['movementSearch','movementType','movementEquipment'])$(id).addEventListener(id==='movementSearch'?'input':'change',renderMovements);
document.querySelectorAll('[data-tab]').forEach(b=>b.addEventListener('click',()=>switchTab(b.dataset.tab)));
document.querySelectorAll('[data-close]').forEach(b=>b.addEventListener('click',()=>$(b.dataset.close).close()));

document.addEventListener('click',e=>{
  const asset=e.target.closest('[data-asset]');
  if(asset&&!e.target.closest('button,a,input,select,textarea'))openDetail(asset.dataset.asset);
  const editC=e.target.closest('[data-edit-consumable]');if(editC)openConsumable(consumableBy(editC.dataset.editConsumable));
  const editA=e.target.closest('[data-edit-asset]');if(editA){$('detailDlg').close();openEquipment(assetBy(editA.dataset.editAsset))}
  const issue=e.target.closest('[data-issue-asset]');if(issue)openMovement('',issue.dataset.issueAsset);
  const link=e.target.closest('[data-link-consumable]');if(link)linkConsumable(link.dataset.linkConsumable);
  const service=e.target.closest('[data-open-service]');if(service)openService(service.dataset.openService);
});
...
function realtime(){
  ...
  const reload=()=>{clearTimeout(reloadTimer);reloadTimer=setTimeout(load,600)};
  let ch=supabase.channel('equipment-workspace-v2');
  for(const table of ['equipment_assets','equipment_consumables','equipment_consumable_links','equipment_consumable_movements','equipment_service_log'])ch=ch.on('postgres_changes',{event:'*',schema:'public',table},reload);
