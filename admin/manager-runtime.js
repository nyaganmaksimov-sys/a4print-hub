(()=>{
  ...
  function start(){
    ...
    const mobileAccessKey='a4print_mobile_access';
    const mobileRefreshKey='a4print_mobile_refresh';
    const mobileExpiresKey='a4print_mobile_expires';
    let customers=[];
    let priceGroups=[];
    ...
    function setStatus(text,state=''){
      ...
      if(!el)return;
      el.textContent=text;
      el.className='manager-status'+(state?' '+state:'');
    }
    ...
    function storedSession(){
      ...
      const access=localStorage.getItem(mobileAccessKey)||'';
      const refresh=localStorage.getItem(mobileRefreshKey)||'';
      const expires=Number(localStorage.getItem(mobileExpiresKey)||0);
      return access?{access_token:access,refresh_token:refresh,expires_at:expires||undefined}:null;
    }
    ...
    function saveSession(next){
      ...
      try{
        ...
        localStorage.setItem(mobileAccessKey,value.access_token);
        if(value.refresh_token)localStorage.setItem(mobileRefreshKey,value.refresh_token);
        if(value.expires_at)localStorage.setItem(mobileExpiresKey,String(value.expires_at));
      }catch{}
    }

    async function fetchJson(url,{method='GET',headers={},body,timeout=22000}={}){
      ...
      try{
        ...
        if(ex?.name==='AbortError'){const e=new Error('Сервер отвечает слишком долго. Повторите попытку.');e.status=408;throw e}
        const e=new Error('Не удалось связаться с A4PRINT HUB. Проверьте интернет.');e.status=0;throw e;
    ...
    async function refreshSession(){
      ...
      refreshPromise=(async()=>{
        ...
        const {r,data}=await fetchJson(`${base}/auth/v1/token?grant_type=refresh_token`,{
          method:'POST',
    ...
    async function request(path,{method='GET',body,prefer,retry=true,timeout=22000}={}){
      ...
      const {r,data}=await fetchJson(base+path,{method,headers,body:body===undefined?undefined:JSON.stringify(body),timeout});
      if(r.status===401&&retry&&await refreshSession())return request(path,{method,body,prefer,retry:false,timeout});
      if(!r.ok){
        const e=new Error(data?.message||data?.hint||data?.details||data?.error_description||data?.error||`HTTP ${r.status}`);
        e.status=r.status;
        throw e;
      }
      return data;
    ...
    function loginUrl(){
      ...
      u.searchParams.set('returnTo',location.pathname+location.search);
      if(new URLSearchParams(location.search).get('mobile')==='1')u.searchParams.set('mobile','1');
      return u.href;
    }

    function handleFatal(ex){
      const msg=ex?.status===401?'Сессия завершена. Войдите снова.':ex?.status===403?'Нет доступа к рабочему столу менеджера.':(ex?.message||'Не удалось открыть рабочий стол.');
      setStatus(msg,'err');
      ...
      if(ex?.status===401)setTimeout(()=>location.replace(loginUrl()),700);
    ...
    function buildPriceGroups(items){
      ...
      [...dedup.values()].filter(isCalculatorItem).forEach(item=>{
        ...
        const baseNameValue=baseName(item.name,tier);
        const cat=categoryFor(baseNameValue);
        if(cat==='3D-ARTPRINT')return;
        const k=`${cat}::${baseNameValue.toLowerCase()}`;
        if(!groups.has(k))groups.set(k,{id:k,category:cat,name:baseNameValue,items:[]});
    ...
    function renderCategories(){
      ...
      $('category').innerHTML=cats.length?cats.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join(''):'<option value="">Нет услуг с ценой</option>';
    ...
    function renderServices(){
      ...
      $('unitPrice').value=0;
      $('serviceHint').textContent=list.length?'Выберите услугу. Цена подставится автоматически из действующего каталога, а для тиражных услуг изменится по количеству.':'В этой категории пока нет услуг с заполненной ценой.';
      calc();
    }
    ...
    setClientType();setPriceMode();calc();
    ...
    $('clientForm').onsubmit=async e=>{
      ...
      const duplicate=customers.find(c=>(phone&&digits(c.phone)===digits(phone))||(email&&String(c.email||'').toLowerCase()===email)||(inn&&digits(c.inn)===digits(inn)));
    ...
    async function verifyAccess(){
      const [profileRaw,isAdmin,isManager]=await Promise.all([
        request('/rest/v1/rpc/get_my_staff_profile',{method:'POST',body:{}}),
        request('/rest/v1/rpc/has_role',{method:'POST',body:{required_role:'ADMIN'}}),
        request('/rest/v1/rpc/has_role',{method:'POST',body:{required_role:'MANAGER'}})
      ]);
      const profile=Array.isArray(profileRaw)?profileRaw[0]:profileRaw;
      if(!isAdmin&&!isManager){const e=new Error('Доступ только для администратора или менеджера.');e.status=403;throw e}
      if(profile?.is_active===false||(profile?.status&&profile.status!=='ACTIVE')){const e=new Error('Учётная запись сотрудника не активна.');e.status=403;throw e}
    ...
    async function refreshAll(){
      ...
      bootPromise=(async()=>{
        ...
        const [priceResult,customersResult,ordersResult]=results;
        if(priceResult.status==='rejected'){
          $('serviceHint').textContent='Не удалось загрузить прайс: '+priceResult.reason.message;
          console.error(priceResult.reason);
        }
        if(customersResult.status==='rejected')$('customers').innerHTML=`<div class="empty-mini">Не удалось загрузить клиентов: ${esc(customersResult.reason.message)}</div>`;
        if(ordersResult.status==='rejected')$('orders').innerHTML=`<div class="empty-mini">Не удалось загрузить заказы: ${esc(ordersResult.reason.message)}</div>`;
        const failed=results.filter(x=>x.status==='rejected').length;
        setStatus(failed?`Работает частично · ошибок: ${failed}`:'Система готова',failed?'warn':'ok');
        return results;
      })().catch(ex=>{handleFatal(ex);throw ex}).finally(()=>{bootPromise=null});
      return bootPromise;
    }

    $('refreshManager').onclick=async()=>{
      const btn=$('refreshManager');btn.disabled=true;btn.textContent='Обновляем…';
