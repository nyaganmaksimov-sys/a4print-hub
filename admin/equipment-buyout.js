...
const state={permissions:[],contracts:[],charges:[],links:[],assets:[],selected:null,loading:false};
...
async function load(){
  ...
  try{
    const [contracts,charges,links,assets]=await Promise.all([
    ...
    for(const result of [contracts,charges,links,assets])if(result.error)throw result.error;
    state.contracts=contracts.data||[];state.charges=charges.data||[];state.links=links.data||[];state.assets=assets.data||[];
