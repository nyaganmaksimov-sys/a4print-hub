...
async function sb(path){
  ...
  const r=await fetch(`${API_BASE}/api/v1/supabase${path}`,{headers:{Accept:'application/json',apikey:SUPABASE_KEY,Authorization:`Bearer ${t}`},cache:'no-store'});
...
function ordersFromDom(){return [...document.querySelectorAll('#recentOrders .order')].length}
async function load(){
