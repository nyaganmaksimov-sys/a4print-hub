...
-- Staff can read internal documents, only equipment managers can mutate this bucket.
drop policy if exists hub_documents_staff_read on storage.objects;
create policy hub_documents_staff_read on storage.objects
...
create policy hub_documents_equipment_manager_insert on storage.objects
...
create policy hub_documents_equipment_manager_update on storage.objects
...
create policy hub_documents_equipment_manager_delete on storage.objects
...
declare
  ...
  v_type text:=case tg_table_name
    when 'equipment_insurance_policies' then 'EQUIPMENT_INSURANCE_POLICY'
    else 'EQUIPMENT_IMPROVEMENT'
  end;
  v_id uuid;
begin
...
create trigger trg_audit_equipment_insurance_policy
after insert or update or delete on public.equipment_insurance_policies
...
create trigger trg_audit_equipment_improvement
after insert or update or delete on public.equipment_improvements
for each row execute function public.audit_equipment_risk_record();
...
begin
  ...
  if v_type='INSURANCE' then
    select p.equipment_id,'Страховой полис '||p.policy_number,p.policy_number
      into v_equipment_id,v_title,v_number
    ...
    select i.equipment_id,i.title,'UPG-'||to_char(i.completed_on,'YYYYMMDD')||'-'||substr(i.id::text,1,8)
      into v_equipment_id,v_title,v_number
  ...
  insert into public.documents(
    document_type_id,status,number,document_date,title,notes,metadata,created_by,created_at,updated_at
  ) values(
    v_document_type_id,'ACTIVE',v_number,current_date,v_title,nullif(btrim(coalesce(p_notes,'')),''),
  ...
  insert into public.document_versions(
    document_id,version_no,file_name,mime_type,file_size,storage_bucket,storage_path,uploaded_by,created_at
  ) values(
    v_document_id,1,btrim(p_file_name),nullif(btrim(coalesce(p_mime_type,'')),''),p_file_size,
    'hub-documents',btrim(p_storage_path),v_actor,clock_timestamp()
  );

  insert into public.document_links(document_id,entity_type,entity_id,note,metadata,created_at)
  values(
    v_document_id,
    case when v_type='INSURANCE' then 'EQUIPMENT_INSURANCE_POLICY' else 'EQUIPMENT_IMPROVEMENT' end,
    p_entity_id,
    nullif(btrim(coalesce(p_notes,'')),''),
    jsonb_build_object('equipment_id',v_equipment_id),clock_timestamp()
  );
...
as $$
  select
    d.id,l.entity_type,l.entity_id,
    nullif(l.metadata->>'equipment_id','')::uuid,
    dt.code,d.title,d.number,d.status,d.document_date,d.notes,
    v.file_name,v.mime_type,v.file_size,v.storage_bucket,v.storage_path,d.created_at
  ...
  left join lateral (
    select dv.file_name,dv.mime_type,dv.file_size,dv.storage_bucket,dv.storage_path
    from public.document_versions dv
    where dv.document_id=d.id
    order by dv.version_no desc,dv.created_at desc
    limit 1
  ) v on true
