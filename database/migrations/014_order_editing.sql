-- A4PRINT HUB: secure order editing from the order card.
-- The same function is deployed to Supabase as migration hub_order_editing.

create or replace function public.update_hub_order_details(
  p_order_id uuid,
  p_customer_id uuid,
...
begin
  ...
  return jsonb_build_object('id',p_order_id,'total',round(v_total,2),'item_count',v_count);
end;
$$;

revoke all on function public.update_hub_order_details(uuid,uuid,text,text,text,text,text,jsonb) from public;
grant execute on function public.update_hub_order_details(uuid,uuid,text,text,text,text,text,jsonb) to authenticated;
