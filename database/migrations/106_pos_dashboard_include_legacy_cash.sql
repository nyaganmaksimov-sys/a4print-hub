-- POS analytics also counts historical MoySklad cash transactions created before POS v2.
...
begin
  ...
  with tin as (
    select ct.* from public.cash_transactions ct
    where ct.organization_id=v_org and ct.direction='INCOME' and ct.created_at>=p_from and ct.created_at<p_to
    ...
    select ct.* from public.cash_transactions ct
    where ct.organization_id=v_org and ct.direction='EXPENSE' and ct.created_at>=p_from and ct.created_at<p_to
      and ct.description ilike 'Возврат МойСклад%'
      and (v_filter is null or ct.created_by=v_filter)
  ), s as (
    ...
    select coalesce(payment_method,'Не указано') k,count(*) n,sum(amount) amount from tin group by 1 order by amount desc
  ), ops as (
    select coalesce(u.full_name,'Неизвестно') k,count(*) n,sum(tin.amount) amount from tin left join public.users u on u.id=tin.created_by group by 1 order by amount desc
  ), items as (
    ...
    select jsonb_agg(jsonb_build_object('id',x.id,'sale_name',coalesce(x.external_id,x.description),'sold_at',x.created_at,'total',x.amount,'payment_method',x.payment_method,'operator',x.operator_name,'customer',x.customer_name) order by x.created_at desc) j
    from (select tin.*,u.full_name operator_name,c.full_name customer_name from tin left join public.users u on u.id=tin.created_by left join public.customers c on c.id=tin.customer_id order by tin.created_at desc limit 20) x
  )
  select jsonb_build_object(
    ...
    'returns_count',(select count(*) from tout),
    'returns_total',coalesce((select sum(amount) from tout),0),
    'net_total',coalesce((select sum(amount) from tin),0)-coalesce((select sum(amount) from tout),0),
