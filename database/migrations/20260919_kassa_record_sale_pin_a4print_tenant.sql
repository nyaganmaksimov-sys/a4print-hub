-- Keep A4PRINT KASSA sale recording on its dedicated POS tenant even when
-- the authenticated administrator belongs to another HUB company.
...
begin
  ...
  if v_admin and p_operator_id is not null and exists(
    select 1 from public.users where id=p_operator_id and is_active=true
  ) then
