-- A4PRINT HUB: Production Farm Phase 46 — explicit owner-payment cash ledger and reversal workflow.

-- Live migration: production_farm_owner_payment_ledger (20260918120301)
create table public.equipment_owner_payment_postings(
  ...
  reversal_transaction_id uuid null unique references public.cash_transactions(id) on delete restrict,
...
revoke all on table public.equipment_owner_payment_postings
from public,anon,authenticated;

create or replace function private.equipment_owner_payment_context(
  p_entity_type text,
  p_entity_id uuid
)
returns jsonb
...
create or replace function public.get_equipment_owner_payment_financial_options(
  p_entity_type text,
  p_entity_id uuid
)
returns jsonb
...
create or replace function public.post_equipment_owner_payment_financial_transaction(
  p_entity_type text,
  p_entity_id uuid,
...
create or replace function public.reverse_equipment_owner_payment_financial_transaction(
  p_entity_type text,
  p_entity_id uuid,
  p_reason text,
  p_reference text,
