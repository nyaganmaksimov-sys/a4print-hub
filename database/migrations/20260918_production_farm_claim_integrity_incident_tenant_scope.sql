-- A4PRINT HUB: Production Farm Phase 31 hardening — tenant scoped integrity incidents.
...
using(
  ...
  and exists(
    ...
    where e.id=equipment_claim_financial_integrity_actions.event_id
...
returns table(
  event_id uuid, claim_id uuid, partner_name text, contract_number text,
  inventory_number text, equipment_name text, detected_at timestamptz,
  event_issue_codes text[], case_state text, latest_note text,
  latest_reference text, latest_at timestamptz, latest_by_name text,
  current_integrity_state text, current_issue_codes text[],
...
begin
  ...
  with current_rows as (
    select x.*,
      public.equipment_claim_integrity_signature(
        x.issue_codes,x.original_transaction_id,x.reversal_transaction_id,
        x.repost_transaction_id,x.actual_net_effect
      ) current_signature
    from public.equipment_condition_claim_financial_integrity_rows() x
  ), incident_rows as (
    select e.id event_id,e.claim_id,e.detected_at,e.issue_codes event_issue_codes,e.issue_signature,
      coalesce(p.legal_name,p.name)::text partner_name,ec.contract_number::text,
      ea.inventory_number::text,ea.name::text equipment_name,
      a.action_type latest_action,a.note latest_note,a.reference latest_reference,
      a.acted_at latest_at,u.full_name::text latest_by_name,
    ...
    left join lateral(
      ...
      where ia.event_id=e.id order by ia.event_sequence desc limit 1
    ) a on true
    left join public.users u on u.id=a.acted_by
    where ea.organization_id=v_org
  )
  select i.event_id,i.claim_id,i.partner_name,i.contract_number,i.inventory_number,
    i.equipment_name,i.detected_at,i.event_issue_codes,
    case when i.latest_action='RESOLVED' and i.current_signature_matches then 'REOPENED'
         else coalesce(i.latest_action,'OPEN') end::text case_state,
  ...
  where coalesce(p_include_resolved,false)
     or (case when i.latest_action='RESOLVED' and i.current_signature_matches then 'REOPENED'
              else coalesce(i.latest_action,'OPEN') end)<>'RESOLVED'
  order by
    case (case when i.latest_action='RESOLVED' and i.current_signature_matches then 'REOPENED'
               else coalesce(i.latest_action,'OPEN') end)
      when 'REOPENED' then 0 when 'OPEN' then 1 when 'INVESTIGATING' then 2
      when 'ACKNOWLEDGED' then 3 else 4 end,
    i.detected_at desc;
end
$$;
...
create or replace function public.set_equipment_claim_integrity_incident_state(
  p_event_id uuid,p_state text,p_note text,p_reference text default null
)
returns uuid
language plpgsql
security definer
...
declare
  ...
  v_current record; v_current_signature text; v_latest text; v_derived text;
  v_actor uuid; v_action_id uuid; v_org uuid; v_event_org uuid;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  if not(public.has_permission('equipment.contracts.manage') or public.has_permission('production.settlements.manage'))
    then raise exception 'PERMISSION_DENIED'; end if;

  v_org:=public.current_user_organization_id();
  if v_org is null then raise exception 'ORGANIZATION_CONTEXT_REQUIRED'; end if;
  if p_event_id is null then raise exception 'INTEGRITY_EVENT_REQUIRED'; end if;
  if p_state not in ('ACKNOWLEDGED','INVESTIGATING','RESOLVED')
    then raise exception 'INTEGRITY_INCIDENT_STATE_INVALID'; end if;
  if nullif(btrim(coalesce(p_note,'')),'') is null or length(btrim(p_note))<3
    then raise exception 'INTEGRITY_INCIDENT_NOTE_REQUIRED'; end if;
  if p_state='RESOLVED' and (nullif(btrim(coalesce(p_reference,'')),'') is null or length(btrim(p_reference))<2)
  ...
  select ea.organization_id into v_event_org
  from public.equipment_condition_claims c
  join public.equipment_assets ea on ea.id=c.equipment_id
  where c.id=v_event.claim_id;
  if v_event_org is distinct from v_org then raise exception 'INTEGRITY_EVENT_NOT_AVAILABLE'; end if;
  ...
  if found then
    v_current_signature:=public.equipment_claim_integrity_signature(
      v_current.issue_codes,v_current.original_transaction_id,
      v_current.reversal_transaction_id,v_current.repost_transaction_id,
  ...
  where a.event_id=v_event.id order by a.event_sequence desc limit 1;

  v_derived:=case when v_latest='RESOLVED' and coalesce(v_current_signature=v_event.issue_signature,false)
                  then 'REOPENED' else coalesce(v_latest,'OPEN') end;
  if v_derived=p_state then raise exception 'INTEGRITY_INCIDENT_STATE_ALREADY_CURRENT'; end if;
  if p_state='RESOLVED' and coalesce(v_current_signature=v_event.issue_signature,false)
    then raise exception 'INTEGRITY_ISSUE_STILL_ACTIVE'; end if;

  v_actor:=public.current_staff_user_id();
  if v_actor is null then raise exception 'STAFF_USER_NOT_FOUND'; end if;

  insert into public.equipment_claim_financial_integrity_actions(event_id,action_type,note,reference,acted_by)
  values(v_event.id,p_state,btrim(p_note),nullif(btrim(coalesce(p_reference,'')),''),v_actor)
...
as $$
declare
  r record; v_signature text; v_event_id uuid; v_inserted boolean;
  v_count integer:=0; v_title text; v_body text; v_latest text; v_org uuid;
begin
  for r in select * from public.equipment_condition_claim_financial_integrity_rows() where integrity_state='ERROR'
  loop
    select ea.organization_id into v_org from public.equipment_assets ea where ea.id=r.equipment_id;
    if v_org is null then continue; end if;

    v_signature:=public.equipment_claim_integrity_signature(
      r.issue_codes,r.original_transaction_id,r.reversal_transaction_id,
      r.repost_transaction_id,r.actual_net_effect
    );
    v_event_id:=null; v_inserted:=false; v_latest:=null;
    ...
    on conflict(claim_id,issue_signature) do nothing returning id into v_event_id;
    ...
    if v_event_id is not null then
      ...
      select a.action_type into v_latest
      from public.equipment_claim_financial_integrity_actions a
      where a.event_id=v_event_id order by a.event_sequence desc limit 1;

      if v_latest='RESOLVED' then
        insert into public.equipment_claim_financial_integrity_actions(event_id,action_type,note,reference,acted_by)
        values(v_event_id,'REOPENED','Повторно обнаружено автоматическим контролем.',null,null);
        v_inserted:=true;
      end if;
    end if;
    ...
    v_count:=v_count+1;
    v_title:=case when v_latest='RESOLVED' then 'Повторное нарушение финансовой цепочки'
                  else 'Нарушение финансовой цепочки требования' end;
    v_body:=coalesce(r.partner_name,'—')||' · договор '||coalesce(r.contract_number,'—')||
      ' · '||coalesce(r.inventory_number,'—')||' · '||array_to_string(r.issue_codes,', ');
    ...
    where u.is_active=true and ou.is_active=true and ou.organization_id=v_org
