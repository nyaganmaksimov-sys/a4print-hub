-- Production Farm Phase 54 — harden timeline RPC.
-- Keep privileged table read in a non-exposed schema and expose only a SECURITY INVOKER wrapper.

create schema if not exists risk_private;

revoke all on schema risk_private from public,anon;
grant usage on schema risk_private to authenticated;

create or replace function risk_private.get_production_farm_risk_action_history(
  p_category text,
  p_entity_id uuid,
...
revoke all on function risk_private.get_production_farm_risk_action_history(text,uuid,integer)
  from public,anon,authenticated;
grant execute on function risk_private.get_production_farm_risk_action_history(text,uuid,integer)
  to authenticated;
...
set search_path=''
as $$
  select risk_private.get_production_farm_risk_action_history(
    p_category,
    p_entity_id,
    p_limit
  )
