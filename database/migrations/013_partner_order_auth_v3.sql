-- A4PRINT HUB: robust Partner Portal order authentication
-- Run after 012_partner_order_auth_fix.sql.

create or replace function public.create_partner_order_v3(
...
begin
  ...
  if v_auth_uid is null then
    begin
      v_auth_uid := nullif((current_setting('request.jwt.claims', true)::jsonb ->> 'sub'),'')::uuid;
    exception when others then
      v_auth_uid := null;
    end;
  end if;
  ...
  values(v_order_id,v_service.name,p_quantity,v_price,v_total,v_parameters);

  return v_order_id;
end;
$$;

grant execute on function public.create_partner_order_v3(uuid,uuid,numeric,text,jsonb,numeric) to authenticated;
