...
do $$
begin
  if not exists (select 1 from pg_constraint where conrelid='public.equipment_owner_settlements'::regclass and conname='equipment_owner_settlements_owner_share_check') then
    alter table public.equipment_owner_settlements add constraint equipment_owner_settlements_owner_share_check check(owner_share_percent between 0 and 100);
  end if;
  if not exists (select 1 from pg_constraint where conrelid='public.equipment_owner_settlements'::regclass and conname='equipment_owner_settlements_hub_share_check') then
    alter table public.equipment_owner_settlements add constraint equipment_owner_settlements_hub_share_check check(hub_share_percent between 0 and 100);
  end if;
end
$$;
...
create trigger trg_audit_equipment_owner_settlements after insert or update or delete on public.equipment_owner_settlements for each row execute function public.audit_row_change();
drop trigger if exists trg_audit_equipment_owner_settlement_lines on public.equipment_owner_settlement_lines;
create trigger trg_audit_equipment_owner_settlement_lines after insert or update or delete on public.equipment_owner_settlement_lines for each row execute function public.audit_row_change();
...
begin
  ...
  if exists(
    ...
    where s.contract_id=v_contract.id and s.status<>'CANCELLED'
  ...
  insert into public.equipment_owner_settlements(
    contract_id,partner_id,period_start,period_end,status,calculation_basis,owner_share_percent,hub_share_percent,direct_costs_before_split,currency,generated_by
  ) values(
    v_contract.id,v_contract.partner_id,p_period_start,p_period_end,'DRAFT',v_contract.calculation_basis,v_contract.owner_share_percent,v_contract.hub_share_percent,v_contract.direct_costs_before_split,v_contract.currency,v_actor
  )
  on conflict(contract_id,period_start,period_end) do update
  set partner_id=excluded.partner_id,calculation_basis=excluded.calculation_basis,owner_share_percent=excluded.owner_share_percent,
      hub_share_percent=excluded.hub_share_percent,direct_costs_before_split=excluded.direct_costs_before_split,currency=excluded.currency,
      generated_by=excluded.generated_by,generated_at=clock_timestamp()
  where public.equipment_owner_settlements.status='DRAFT'
  returning * into v_settlement;

  if v_settlement.id is null or v_settlement.status<>'DRAFT' then raise exception 'SETTLEMENT_LOCKED'; end if;
  ...
  insert into public.equipment_owner_settlement_lines(
    settlement_id,production_job_id,equipment_id,order_id,completed_at,operation_revenue,received_revenue,direct_costs,split_base,owner_amount,hub_amount
  )
  select
    ...
    round(greatest(coalesce(j.production_cost,0),0),2),0,0,0
  ...
  left join lateral(
    ...
    from public.payments p where p.order_id=j.order_id
  ) pay on true
  left join lateral(
    select coalesce(sum(greatest(coalesce(j2.operation_amount,0),0)),0) as order_operations
    from public.production_jobs j2 where j2.order_id=j.order_id and j2.status<>'CANCELLED'
  ) op on true
  where j.status='DONE' and j.completed_at is not null and j.completed_at::date between p_period_start and p_period_end
    and (ca.starts_on is null or j.completed_at::date>=ca.starts_on) and (ca.ends_on is null or j.completed_at::date<=ca.ends_on)
    and j.completed_at::date>=v_contract.starts_on and (v_contract.ends_on is null or j.completed_at::date<=v_contract.ends_on)
    and not exists(
      select 1 from public.equipment_owner_settlement_lines old_l
      join public.equipment_owner_settlements old_s on old_s.id=old_l.settlement_id
      where old_l.production_job_id=j.id and old_s.id<>v_settlement.id and old_s.status in('APPROVED','PAID')
    );
  ...
  set split_base=round(greatest(0,
    ...
    - case when v_contract.calculation_basis='NET_AFTER_DIRECT_COSTS' or v_contract.direct_costs_before_split then l.direct_costs else 0 end
  ),2)
  ...
  update public.equipment_owner_settlements s
  set gross_revenue=x.gross_revenue,received_revenue=x.received_revenue,direct_costs=x.direct_costs,split_base=x.split_base,
      owner_amount=x.owner_amount,hub_amount=x.hub_amount,owner_share_percent=v_contract.owner_share_percent,
      hub_share_percent=v_contract.hub_share_percent,direct_costs_before_split=v_contract.direct_costs_before_split,generated_at=clock_timestamp()
  from(
    select coalesce(sum(operation_revenue),0)::numeric gross_revenue,coalesce(sum(received_revenue),0)::numeric received_revenue,
           coalesce(sum(direct_costs),0)::numeric direct_costs,coalesce(sum(split_base),0)::numeric split_base,
           coalesce(sum(owner_amount),0)::numeric owner_amount,coalesce(sum(hub_amount),0)::numeric hub_amount
    from public.equipment_owner_settlement_lines where settlement_id=v_settlement.id
  ) x
  where s.id=v_settlement.id returning s.* into v_settlement;
  return v_settlement;
end;
$$;
...
create or replace function public.set_equipment_owner_settlement_status(p_settlement_id uuid,p_status text,p_payment_reference text default null,p_notes text default null)
returns public.equipment_owner_settlements
language plpgsql
security definer
set search_path=public
...
begin
  ...
  if v_target='APPROVED' then
    ...
    if not exists(select 1 from public.equipment_owner_settlement_lines l where l.settlement_id=v_row.id) then raise exception 'SETTLEMENT_HAS_NO_JOBS'; end if;
    if exists(
      select 1 from public.equipment_owner_settlement_lines l
    ...
    if v_row.owner_amount>0 and nullif(trim(coalesce(p_payment_reference,'')),'') is null then raise exception 'PAYMENT_REFERENCE_REQUIRED'; end if;
...
drop view if exists public.equipment_owner_settlement_overview;
create view public.equipment_owner_settlement_overview
with (security_invoker=true)
as
select s.*,p.name as partner_name,c.contract_number,
