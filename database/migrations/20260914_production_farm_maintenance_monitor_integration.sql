-- Phase 12: integrate recurring equipment maintenance plans into production monitoring.
-- Reuse the canonical equipment_maintenance_plans calendar; do not create a parallel maintenance model.
...
alerts as (
  select
    ...
    array_remove(array[
      case when (
        (m.next_service_date is not null and m.next_service_date<current_date)
        or exists (
          select 1 from public.equipment_maintenance_plans p
          where p.equipment_id=m.equipment_id
            and p.is_active
            and p.next_due_date<current_date
        )
      ) then 'SERVICE_OVERDUE' end,
      case when (
        (m.next_service_date is not null and m.next_service_date>=current_date and m.next_service_date<=current_date+m.service_due_days)
        or exists (
          select 1 from public.equipment_maintenance_plans p
          where p.equipment_id=m.equipment_id
            and p.is_active
            and p.next_due_date>=current_date
            and p.next_due_date<=current_date+p.remind_before_days
        )
      ) then 'SERVICE_DUE' end,
...
select
  a.*,
  cardinality(a.alerts) as alert_count,
  mp.id as maintenance_plan_id,
  mp.title as maintenance_plan_title,
  mp.service_type as maintenance_plan_type,
  mp.next_due_date as maintenance_plan_due_date,
  mp.remind_before_days as maintenance_remind_before_days
from alerts a
left join lateral (
  select p.id,p.title,p.service_type,p.next_due_date,p.remind_before_days
  from public.equipment_maintenance_plans p
  where p.equipment_id=a.equipment_id and p.is_active
  order by p.next_due_date,p.id
  limit 1
) mp on true;

comment on view public.production_equipment_monitor is
  'Configurable equipment production-health monitor over OEE, canonical capacity rules, recurring maintenance plans, legacy service deadlines and margin. Advisory read-only signals.';
