...
with q as (
  ...
  union all
  select 'document'::text,d.id,coalesce(nullif(d.title,''),nullif(d.document_number,''),'Документ')::text,
    concat_ws(' · ',nullif(d.document_number,''),d.status::text,case when o.order_number is not null then 'Заказ №'||o.order_number::text end,coalesce(nullif(c.full_name,''),nullif(c.company_name,'')))::text,
    lower(concat_ws(' ',d.document_number,d.title,d.status::text,d.notes,o.order_number::text,c.full_name,c.company_name)),'./documents.html'::text,
    jsonb_build_object('document_id',d.id,'document_number',d.document_number,'title',d.title,'status',d.status::text,'issue_date',d.issue_date,'valid_until',d.valid_until,'customer_id',d.customer_id,'customer_name',coalesce(nullif(c.full_name,''),nullif(c.company_name,'')),'order_id',d.order_id,'order_number',o.order_number)
  from public.documents d left join public.customers c on c.id=d.customer_id left join public.orders o on o.id=d.order_id
  union all
