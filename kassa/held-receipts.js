(()=>{
  ...
  function stageLabel(row){
    if(row.stage==='backend_done')return 'Создан в МойСклад · сохраняется в HUB';
    if(row.last_error)return 'Ошибка отправки';
  ...
  function installStyles(){
    ...
    const st=document.createElement('style');st.id='a4HeldStyle';st.textContent=`
      #utilityDrawer.a4-held-mode .utility-card{width:min(920px,calc(100vw - 255px));max-width:none}
      #utilityDrawer.a4-held-mode .utility-body{background:#f5f8fa;padding:18px 20px}
  ...
  function openDrawer(){
    ...
    drawer.classList.remove('a4-history-mode');drawer.classList.add('a4-held-mode');
    $('utilityTitle').textContent='Отложенные чеки';
    $('utilitySubtitle').textContent='Локальная очередь · отправка в МойСклад и A4PRINT HUB';
    drawer.classList.add('open');drawer.setAttribute('aria-hidden','false');
    return true;
  }

  async function rows(){
    const list=DB?await DB.getAll('queue'):[];
  ...
  function itemNames(row){
    ...
    return list.slice(0,4).map(i=>`${esc(i.name||'Позиция')} × ${Number(i.qty||0).toLocaleString('ru-RU')}`).join(' · ')+(list.length>4?` · ещё ${list.length-4}`:'');
  }
  ...
  async function render(){
    ...
    const errors=list.filter(x=>x.last_error).length;
    const inHub=list.filter(x=>x.stage==='backend_done').length;
    body.innerHTML=`
      <div class="a4-held-summary">
        <div class="a4-held-stat"><span>В очереди</span><strong>${list.length}</strong></div>
        <div class="a4-held-stat"><span>Сумма чеков</span><strong>${money(total)}</strong></div>
        <div class="a4-held-stat"><span>Требуют внимания</span><strong>${errors}${inHub?` · ${inHub} на финальном этапе`:''}</strong></div>
      </div>
      <div class="a4-held-actions"><button id="a4HeldRetry" class="primary" type="button" ${list.length?'':'disabled'}>↻ Повторить синхронизацию</button><button id="a4HeldRefresh" type="button">Обновить список</button></div>
    ...
    $('a4HeldRefresh')?.addEventListener('click',render);
    $('a4HeldRetry')?.addEventListener('click',retry);
  }

  async function retry(){
    const btn=$('a4HeldRetry');if(btn){btn.disabled=true;btn.textContent='Синхронизация…'}
    $('retryQueue')?.click();
    ...
    pollTimer=setInterval(async()=>{
      ticks++;
      await render().catch(()=>{});
      const list=await rows().catch(()=>[]);
      if(!list.length||ticks>=20){clearInterval(pollTimer);pollTimer=null}
    },750);
  }

  function openHeld(event){
    ...
    $('navHeld')?.classList.add('active');
    const body=$('utilityBody');if(body)body.innerHTML='<div class="a4-held-empty">Читаю локальную очередь…</div>';
    render().catch(e=>{if(body)body.innerHTML=`<div class="a4-held-empty"><b>Не удалось открыть очередь</b><span>${esc(e?.message||e)}</span></div>`});
  }

  document.addEventListener('click',event=>{
    if(event.target?.closest?.('#navHeld'))openHeld(event);
    else if(event.target?.closest?.('#navSettings,#navHelp,#navHistory'))$('utilityDrawer')?.classList.remove('a4-held-mode');
  },true);
})();
