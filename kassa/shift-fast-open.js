(()=>{
  ...
  async function waitForConfirmed(timeoutMs=20000){
  ...
  window.fetch=async function a4FastShiftFetch(input,init={}){
    const u=shiftUrl(input),method=methodOf(input,init);
    if(!u)return previousFetch(input,init);
    ...
    if(method==='POST'&&u.pathname.endsWith('/close')&&(state.local||isLocalShift(gate?.remoteShift))&&!state.confirmed){
      ...
      state.local=null;DB?.setMeta?.('shift',null).catch(()=>{});emit('local-close',{local:true});
      (async()=>{
        try{
          await waitForConfirmed(20000);
      ...
      const patched=function(fn,params,options){
        ...
        return (async()=>{
          ...
          if(!current||current.startsWith('local-')||isLocalShift(state.local)){
            const remote=await waitForConfirmed(20000);
