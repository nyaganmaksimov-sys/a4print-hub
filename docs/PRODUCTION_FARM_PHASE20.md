# Production Farm Phase 20 — сроки договоров оборудования

Phase 20 контролирует приближение и наступление даты окончания договоров партнёрского оборудования. Система **не продлевает и не расторгает договор автоматически**: юридическое действие остаётся отдельным управляемым процессом Phase 14.
...
Рабочая Supabase повторно проверена транзакционным тестом с `ROLLBACK`: два партнёра увидели только свои договоры, договоры с окончанием через 7 и 14 дней получили `ENDS_7` и `ENDS_14`, первый запуск уведомлений создал событие, повторный запуск не создал дубль. Результат: `phase20_contract_deadlines_ok`.
