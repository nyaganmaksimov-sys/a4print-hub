...
- отдельный `admin/storage-network-fix.js` удалён после переноса его нужной функции в канонический `admin/config.js`: Supabase Storage (`/storage/v1`) теперь проходит через общий HUB API routing с длинным файловым timeout и direct fallback; backend маршрут обслуживает `api/src/supabase-storage-proxy.js`;
...
`admin/partners-api-fallback.js` пока намеренно остаётся как маленький compatibility shim для старых закешированных клиентов. Текущий `admin/config.js` его не загружает; удалять его имеет смысл только после достаточного cache-transition периода.
