# Production Farm — Phase 45: allocation weight annex

Phase 45 добавляет юридически контролируемое изменение `allocation_weight` оборудования в действующем договоре.
...
До Phase 45 легального workflow изменения `allocation_weight` не было.
...
`phase45_allocation_weight_annex_ok`.
