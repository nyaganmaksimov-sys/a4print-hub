...
function note(text,error=false){const n=$('notice');n.textContent=text;n.className='notice'+(error?' error':'');n.style.display='block';clearTimeout(note.t);note.t=setTimeout(()=>n.style.display='none',3200)}
...
async function loadCore(){
  ...
  const [g,ac,op]=await Promise.all([
    ...
    supabase.from('cash_accounts').select('id,name,account_type,is_active').eq('organization_id',org.id).eq('is_active',true).order('name'),
  ...
  $('operatorSelect').innerHTML=operators.map(x=>`<option value="${x.id}">${esc(x.full_name)}${x.is_self?' · вы':''}</option>`).join('');
...
async function pay(){
  ...
  const total=cart.reduce((s,x)=>s+x.price*x.qty,0),items=cart.map(x=>({...x}));
  $('pay').disabled=true;$('pay').textContent='Проводим продажу…';
  try{
    const d=await api('/api/v1/pos/sale',{method:'POST',body:JSON.stringify({items,payment_method:$('method').value,customer_id:customer?.id||null,operator_id:selectedOperatorId||null})});
